...
void memcpyTest::processTest()
{
	...
	{
		char test[4] = {1, 2, 3, 4};
		testThisFunAndVals(baseFunction, testFunction, testValsFun, testValForBase, mkSpMemVal(test, sizeof(test), "src"), mkSpBaseVal<size_t>(4, "n"));
		...
		testThisFunAndVals(baseFunction, testFunction, testValsFun, testValForBase, mkSpMemVal(nullptr, 0, "src"), mkSpBaseVal<size_t>(4, "n"));
	}
